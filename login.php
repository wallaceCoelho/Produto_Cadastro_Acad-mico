<?php
    ...
    $conn = new mysqli($server,$usuario,$senha,$bd); // $conn armazena as informações (Conexão)
    if(!$conn){
        ...
        if(isset($_POST['email'])){
            ...
            if($res -> num_rows > 0){
                //Usuario existe e agora??
                $user = $res->fetch_object();
    
                    $_SESSION['nome'] = $user->nome;
	                $_SESSION['email'] = $user->email;
	                $_SESSION['dt_nasc'] = $user->dt_nasc;
    
                header('location: cadastrarcateg.php');
